...
const dbInit = {
	...
	async v3_5DB(c) {
		try {
			await c.env.db.prepare(`ALTER TABLE account ADD COLUMN forward_signature TEXT NOT NULL DEFAULT '';`).run();
		} catch (e) {
	...
	async v3_4DB(c) {
		try {
			await c.env.db.batch([
				c.env.db.prepare(`ALTER TABLE email ADD COLUMN spam INTEGER NOT NULL DEFAULT 0;`),
				c.env.db.prepare(`ALTER TABLE setting ADD COLUMN spam_from TEXT NOT NULL DEFAULT '';`),
				c.env.db.prepare(`ALTER TABLE setting ADD COLUMN spam_subject TEXT NOT NULL DEFAULT '';`)
			]);
		} catch (e) {
			console.warn(`跳过字段：${e.message}`);
		}
	...
	async v3_3DB(c) {
		try {
			await c.env.db.prepare(`ALTER TABLE email ADD COLUMN scheduled_at TEXT;`).run();
		} catch (e) {
			console.warn(`跳过字段：${e.message}`);
		}
	},
